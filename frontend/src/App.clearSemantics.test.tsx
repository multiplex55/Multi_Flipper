...
vi.mock("@/components/RadiusRouteWorkspace", () => ({ RadiusRouteWorkspace: ({ radiusScanSession }: { radiusScanSession?: { results?: unknown[] } }) => <div data-testid="route-session-count">{radiusScanSession?.results?.length ?? 0}</div> }));
...
describe("App clear semantics", () => {
  it("only explicit reset clears radius session", async () => {
    render(<App />);
    ...
    expect(await screen.findByTestId("route-session-count")).toHaveTextContent("1");

    fireEvent.click(screen.getByRole("tab", { name: "tabRadius" }));
    expect(screen.getByTestId("rows-radius")).toHaveTextContent("1");
    ...
    fireEvent.click(screen.getByRole("tab", { name: "tabRoute" }));
    expect(await screen.findByTestId("route-session-count")).toHaveTextContent("0");
  });
});
