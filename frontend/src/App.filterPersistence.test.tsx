...
vi.mock("@/components/RadiusRouteWorkspace", () => ({ RadiusRouteWorkspace: () => <div data-testid="route-workspace" /> }));
vi.mock("@/components/ScanResultsTable", () => ({
  ScanResultsTable: ({ tradeStateTab, results }: { tradeStateTab?: string; results: unknown[] }) => {
    const [hideAll, setHideAll] = useState(false);
    if (tradeStateTab !== "radius") return <div data-testid={`rows-${tradeStateTab}`}>{results.length}</div>;
    return (
      <div>
        <div data-testid="radius-visible-count">{hideAll ? 0 : results.length}</div>
        <div data-testid="radius-raw-count">{results.length}</div>
        <button type="button" onClick={() => setHideAll((prev) => !prev)}>toggle-hide</button>
      </div>
    );
  },
}));
...
describe("App filter persistence", () => {
  it("local filters do not look like data loss across tab changes", async () => {
    render(<App />);

    fireEvent.click(await screen.findByRole("button", { name: "scan" }));
    await waitFor(() => expect(screen.getByTestId("radius-raw-count")).toHaveTextContent("1"));

    fireEvent.click(screen.getByRole("button", { name: "toggle-hide" }));
    expect(screen.getByTestId("radius-visible-count")).toHaveTextContent("0");
    expect(screen.getByTestId("radius-raw-count")).toHaveTextContent("1");

    fireEvent.click(screen.getByRole("tab", { name: "tabRoute" }));
    fireEvent.click(screen.getByRole("tab", { name: "tabRadius" }));

    expect(await screen.findByTestId("radius-visible-count")).toHaveTextContent("0");
    expect(screen.getByTestId("radius-raw-count")).toHaveTextContent("1");
  });
});
